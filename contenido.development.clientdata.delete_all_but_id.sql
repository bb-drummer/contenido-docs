--
-- remove "ALL" but selected client-id's CONTENIDO client data from database tables
--
-- !!! ATTENTION !!!
...
DELETE `con_frontendgroups` FROM `con_frontendgroups`WHERE `con_frontendgroups`.`idclient` <> '{IDCLIENT}';
DELETE `con_frontendusers` FROM `con_frontendusers` WHERE `con_frontendusers`.`idclient` <> '{IDCLIENT}';
...
-- the client main records themself
